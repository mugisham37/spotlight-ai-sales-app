...
export class SessionSecurityMonitor {
  ...
  private static async getLastKnownLocation(
    ...
    // In a real implementation, this would query user's location history from database
    // For now, log the check and return null
    structuredLogger.logSecurity({
      level: LogLevel.INFO,
      message: "Last known location check performed",
      requestId: crypto.randomUUID(),
      userId,
      eventType: "ip_location_change",
      severity: "low",
      metadata: {
        checkType: "last_known_location",
        timestamp: new Date().toISOString(),
      },
    });
  ...
  private static async updateUserLocation(
    ...
    // In production, this would update the user's location in the database
    structuredLogger.logSecurity({
      level: LogLevel.INFO,
  ...
  private static async hasLocationChanged(
    ...
    // to compare the geographic locations of IPs

    // For now, we'll do a simple IP comparison and log the check
    structuredLogger.logSecurity({
      level: LogLevel.INFO,
      message: "Location change check performed",
      requestId: crypto.randomUUID(),
      eventType: "ip_location_change",
      severity: "low",
      metadata: {
        lastLocation,
        currentIp,
        locationChanged: lastLocation !== currentIp,
        timestamp: new Date().toISOString(),
      },
    });

    // Simple comparison - in production, this would use geolocation APIs
    return lastLocation !== currentIp;
  }

  private static async getLastKnownDevice(
    userId: string
  ): Promise<string | null> {
    // In a real implementation, this would query user's device history from database
    // For now, log the check and return null
    structuredLogger.logSecurity({
      level: LogLevel.INFO,
      message: "Last known device check performed",
      requestId: crypto.randomUUID(),
      userId,
      eventType: "device_change",
      severity: "low",
      metadata: {
        checkType: "last_known_device",
        timestamp: new Date().toISOString(),
      },
    });
  ...
  private static async updateUserDevice(
    ...
    // In production, this would update the user's device information in the database
    structuredLogger.logSecurity({
      level: LogLevel.INFO,
  ...
  private static async getRecentSessionCount(userId: string): Promise<number> {
    // In a real implementation, this would count recent sessions from database
    // For now, we'll simulate by checking our in-memory sessions
    try {
      const oneMinuteAgo = new Date(Date.now() - 60000);
      const recentSessions = Array.from(this.sessions.values()).filter(
        (session) =>
          session.userId === userId &&
          session.createdAt > oneMinuteAgo &&
          session.isActive
      );

      structuredLogger.logSecurity({
        level: LogLevel.INFO,
        message: "Recent session count check performed",
        requestId: crypto.randomUUID(),
        userId,
      ...
      console.error("Error counting recent sessions:", error);
      return 0;
    }
  }
